...
public class MapEspecialActivity extends FragmentActivity implements OnMapReadyCallback {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_map_especial);
    ...
    public void onMapReady(GoogleMap googleMap) {
        ...
        myRef.child("propio/"+getIntent().getStringExtra("tipo")).addListenerForSingleValueEvent(new ValueEventListener() {
            @Override
            public void onDataChange(DataSnapshot dataSnapshot) {
                for(DataSnapshot s: dataSnapshot.getChildren()){
                    Parada p = s.getValue(Parada.class);
                    if(primera){
                        nombreDestino = s.getKey();
                        primera = false;
                    }
                    desti = new LatLng(p.getLatitud(),p.getLongitud());
                    ...
                    marcadores.put(s.getKey(),destinoAzul);
                    paradas.put(s.getKey(),p);
                    num ++;
                }

                desti = new LatLng(paradas.get(nombreDestino).getLatitud(),paradas.get(nombreDestino).getLongitud());

            }

            @Override
            public void onCancelled(DatabaseError databaseError) {
                Toast.makeText(MapEspecialActivity.this, " Erro: se interumpio la operación", Toast.LENGTH_SHORT).show();
            }
        });
    ...
    private void localizacion(){
        ...
        if(permissionCheck == 0){
            ...
            SettingsClient client	 =	LocationServices.getSettingsClient(MapEspecialActivity.this);
            Task<LocationSettingsResponse> task	=	client.checkLocationSettings(builder.build());
            task.addOnSuccessListener(MapEspecialActivity.this,	 new	OnSuccessListener<LocationSettingsResponse>()
            {
                @Override
            ...
            task.addOnFailureListener(MapEspecialActivity.this,	 new	OnFailureListener()	 {
                @Override
                    ...
                    switch	(statusCode)	{
                        case	CommonStatusCodes.RESOLUTION_REQUIRED:
                            ...
                            try	{//	Show	the	dialog	by	calling	startResolutionForResult(),	and	check	the	result	in	onActivityResult().
                                ...
                                resolvable.startResolutionForResult(MapEspecialActivity.this,
    ...
    private void enMovimiento(){
        ...
        if(complete) {
            ...
            try {
                ...
                if (results.routes.length > 0) {
                    if (results.routes.length > routeSelected) {
                        ...
                        if(cambio){
                            ...
                            StringTokenizer tok = new StringTokenizer(tim," ");
                            int t = Integer.parseInt(tok.nextToken());
                            tiempototal = tiempototal + t;

                            String kim = results.routes[0].legs[0].distance.humanReadable;
                            tok = new StringTokenizer(kim," ");
                            double td = Double.parseDouble(tok.nextToken());
                            kmtotal = kmtotal+td;
                            cambio = false;
                        }
                        if(comienzo ){
                            ...
                            kmtotal = Double.parseDouble(tok.nextToken());
                            String tim = results.routes[0].legs[0].duration.humanReadable;
                            tok = new StringTokenizer(tim," ");
                            tiempototal = Integer.parseInt(tok.nextToken());
                            re.setTipo("Especial");
                ...
                e.printStackTrace();
                Toast.makeText(MapEspecialActivity.this, "Error con el servidor", Toast.LENGTH_SHORT).show();
            } catch (InterruptedException e) {
                recorriendo = false;
                tiempo.setText("");
                distanci.setText("Vuelva a iniciar recorrido");
                e.printStackTrace();
                Toast.makeText(MapEspecialActivity.this, "Se perdió conexión", Toast.LENGTH_SHORT).show();
            } catch (IOException e) {
                recorriendo = false;
                tiempo.setText("");
                distanci.setText("Vuelva a iniciar recorrido");
                e.printStackTrace();
                Toast.makeText(MapEspecialActivity.this, "Error en la ruta", Toast.LENGTH_SHORT).show();
            }
    ...
    private void llegue(Long x){
        if(x<=65){
            paradas.remove(nombreDestino);
            marcadores.get(nombreDestino).setVisible(false);
            if(paradas.isEmpty()) {
                recorriendo = false;
                tiempo.setText("");
                distanci.setText("¡Has Terminado!");
                removePolyline();
                re.setTiempo(tiempototal);
                re.setKm(kmtotal);
                StringTokenizer tok = new StringTokenizer(getIntent().getStringExtra("tipo"),"/");
                tok.nextToken();
                re.setDestino(tok.nextToken());
                registrarRecorrido();
                kmDestino = 0;
                primeraKm = true;
                ...
                boolean pri = true;
                for (Map.Entry<String, Parada> entry : paradas.entrySet()) {
                    if(pri){
                        nombreDestino = entry.getKey();
                        desti = new LatLng(paradas.get(entry.getKey()).getLatitud(),
                                paradas.get(entry.getKey()).getLongitud());
                        pri = false;
                        cambio = true;
                    }
                }
    ...
    private void updateLocalizacion(){
        myRef.child("ubicaciones/" + user.getUid()).addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            @Override
            public void onCancelled(DatabaseError databaseError) {
                Toast.makeText(MapEspecialActivity.this, "ERROR subiendo a base de datos" + databaseError
    ...
    private void getUsersNear(){
        myRef.child("ubicaciones/").addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            @Override
            public void onCancelled(DatabaseError databaseError) {
                Toast.makeText(MapEspecialActivity.this, "ERROR cargando usuarios cercanos" + databaseError
                        .getMessage().toString(), Toast.LENGTH_SHORT).show();
            }
        });
    ...
    private void iniciarKm(){
        myRef.child("users/"+user.getUid()).addListenerForSingleValueEvent(new	ValueEventListener(){
            ...
            @Override
            public void onCancelled(DatabaseError databaseError)	{
                Toast.makeText(MapEspecialActivity.this, "Error descargando datos de usuario "+
